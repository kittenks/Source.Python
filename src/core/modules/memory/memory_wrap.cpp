...
void export_register_t(scope _memory)
{
	enum_<Register_t>("Register")
		...
		*/
		...
		.value("SPL", SPL).value("BPL", BPL)
		.value("SIL", SIL).value("DIL", DIL)
		.value("R8B", R8B).value("R9B", R9B)
		.value("R10B", R10B).value("R11B", R11B)
		.value("R12B", R12B).value("R13B", R13B)
		.value("R14B", R14B).value("R15B", R15B)
		.value("R8W", R8W).value("R9W", R9W)
		.value("R10W", R10W).value("R11W", R11W)
		.value("R12W", R12W).value("R13W", R13W)
		.value("R14W", R14W).value("R15W", R15W)
		.value("R8D", R8D).value("R9D", R9D)
		.value("R10D", R10D).value("R11D", R11D)
		.value("R12D", R12D).value("R13D", R13D)
		.value("R14D", R14D).value("R15D", R15D)
		.value("RAX", RAX).value("RCX", RCX)
		.value("RDX", RDX).value("RBX", RBX)
		.value("RSP", RSP).value("RBP", RBP)
		.value("RSI", RSI).value("RDI", RDI)
		.value("R8", R8).value("R9", R9)
		.value("R10", R10).value("R11", R11)
		.value("R12", R12).value("R13", R13)
		.value("R14", R14).value("R15", R15)
		.value("XMM8", XMM8).value("XMM9", XMM9)
		.value("XMM10", XMM10).value("XMM11", XMM11)
		.value("XMM12", XMM12).value("XMM13", XMM13)
		.value("XMM14", XMM14).value("XMM15", XMM15)
#endif
	;
}
...
void export_registers(scope _memory)
{
	class_<CRegisters, boost::noncopyable>("Registers", no_init)
		...
		*/
#ifdef SOURCEPYTHON_X86_64
		.def_readonly("spl", &CRegisters::m_spl)
		.def_readonly("bpl", &CRegisters::m_bpl)
		.def_readonly("sil", &CRegisters::m_sil)
		.def_readonly("dil", &CRegisters::m_dil)
		.def_readonly("r8", &CRegisters::m_r8)
		.def_readonly("r9", &CRegisters::m_r9)
		.def_readonly("r10", &CRegisters::m_r10)
		.def_readonly("r11", &CRegisters::m_r11)
		.def_readonly("r12", &CRegisters::m_r12)
		.def_readonly("r13", &CRegisters::m_r13)
		.def_readonly("r14", &CRegisters::m_r14)
		.def_readonly("r15", &CRegisters::m_r15)
		.def_readonly("rax", &CRegisters::m_rax)
		.def_readonly("rcx", &CRegisters::m_rcx)
		.def_readonly("rdx", &CRegisters::m_rdx)
		.def_readonly("rbx", &CRegisters::m_rbx)
		.def_readonly("rsp", &CRegisters::m_rsp)
		.def_readonly("rbp", &CRegisters::m_rbp)
		.def_readonly("rsi", &CRegisters::m_rsi)
		.def_readonly("rdi", &CRegisters::m_rdi)
		.def_readonly("xmm8", &CRegisters::m_xmm8)
		.def_readonly("xmm9", &CRegisters::m_xmm9)
		.def_readonly("xmm10", &CRegisters::m_xmm10)
		.def_readonly("xmm11", &CRegisters::m_xmm11)
		.def_readonly("xmm12", &CRegisters::m_xmm12)
		.def_readonly("xmm13", &CRegisters::m_xmm13)
		.def_readonly("xmm14", &CRegisters::m_xmm14)
		.def_readonly("xmm15", &CRegisters::m_xmm15)
#endif
	;
}
